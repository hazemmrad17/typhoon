...
export function TyphoonNavbar({ current }: { current?: 'home' | 'faq' | 'contact' }) {
  ...
  /* ── Pilule unique : logo + liens + CTA, façon topbar /zone ──
     Remplace l'ancienne navbar Webflow (2 niveaux, hamburger, overlay) par
     un seul conteneur pill. Les contrôles (thème + accent) restent à côté,
     hors de la pilule. */
  return (
    <div className="navbar landingpage">
      <div className="webflow-native-navbar w-nav" role="banner">
        <Link
          to="/"
        ...
        <nav aria-label="Navigation principale" className="nav-menu-wrapper w-nav-menu">
