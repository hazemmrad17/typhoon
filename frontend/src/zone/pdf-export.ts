...
const DISCLAIMER = (
  'Ce document est généré automatiquement par Typhoon à partir de données ' +
...
export async function exportInsurerPdf(input: InsurerPdfInput): Promise<void> {
  ...
  doc.text(`Généré par Typhoon le ${report.date_generation}`, M, y);
  y += 4;
  doc.setDrawColor(LINE);
  doc.setLineWidth(0.35);
  doc.line(M, y, PAGE_W - M, y);
  ...
  for (let i = 1; i <= total; i++) {
    ...
    doc.setFontSize(8);
    doc.setTextColor(MUTED);
    doc.text('Généré par Typhoon · Sources : Géorisques (BRGM/MTE), BDNB, Open-Meteo, Copernicus CDS', M, FOOTER_TOP + 5);
    doc.text(`Page ${i} / ${total}`, PAGE_W - M, FOOTER_TOP + 5, { align: 'right' });
  }
...
export async function exportPortfolioPdf(input: PortfolioPdfInput): Promise<void> {
  ...
  if (wordmark) {
    doc.addImage(wordmark, 'PNG', MM, 10, 42, 42 / (766.43 / 140.93));
  } else {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(20);
    doc.setTextColor('#FFFFFF');
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(MUTED);
    doc.text('Généré par Typhoon · Sources : Géorisques (BRGM/MTE), BDNB, Open-Meteo, Copernicus CDS', MM, FOOTER_Y + 4);
